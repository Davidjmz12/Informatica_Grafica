...
int main()
{
    ...
    // TEST POINT
    t.addTest("1",Test::EXPECT_EQ(canonical.coord_into_canonical(Geometric::vector(0,0,0)),Geometric::vector0()));
    t.addTest("2",Test::EXPECT_EQ(canonical.coord_from_canonical(Geometric::vector(0,0,0)),Geometric::vector0()));
    ...
    // TEST MATRIX
    t.addTest("3",Test::EXPECT_EQ(canonical.canonical_to_base(),LinearMap::identity()));
